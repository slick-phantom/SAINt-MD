...
class MegaSessionManager {
    constructor() {
        this.sessionId = process.env.SESSION_ID; // Changed from MEGA_ID to SESSION_ID
    ...
    async initialize() {
        try {
            ...
            console.log(`Looking for SESSION ID: ${this.sessionId}`);
            ...
            // Find the folder with our SESSION_ID
            const idFolder = await this.findFolder(storage, this.sessionId);
    ...
    async findFolder(storage, folderName) {
        // Look for the folder with our SESSION_ID
